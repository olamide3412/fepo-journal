  ...
  <nav class="fixed top-0 left-0 w-full z-50 shadow bg-light text-black dark:bg-dark dark:text-white">
    <div class="container flex items-center justify-between p-2">
      <Link href="/" class="flex items-center space-x-2">
        ...
        <div>
          <h1 class="text-primary dark:text-secondary-200 font-bold text-lg">
            Maibere Ventures
            <span class="block text-xs text-gray-600 dark:text-gray-400">Nigeria Ltd</span>
          </h1>
        </div>
      </Link>
      ...
      <ul class="hidden md:flex space-x-4">
        ...
        <NavLink :href="route('products')" :active="$page.component === 'Products'">Farm Produce</NavLink>
        <NavLink :href="route('livestock')" :active="$page.component === 'Livestock'">Livestock</NavLink>
    ...
    <transition name="slide">
      <div v-if="isOpen" class="md:hidden shadow-md p-4">
        <ul class="space-y-3 text-center">
          ...
          <li><Link :href="route('products')" @click="toggle">Farm Produce</Link></li>
          <li><Link :href="route('livestock')" @click="toggle">Livestock</Link></li>
