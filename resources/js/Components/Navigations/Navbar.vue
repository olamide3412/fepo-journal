  ...
  <nav class="fixed top-0 left-0 w-full z-50 shadow bg-light text-black dark:bg-dark dark:text-white">
    <div class="container flex items-center justify-between p-2">
      <Link href="/" class="flex items-center space-x-1">
        ...
        <div>
          <h1 class="text-primary dark:text-secondary-200 font-bold text-lg">
            FEPO
            <span class="block text-xs text-gray-600 dark:text-gray-400">Journal</span>
          </h1>
        </div>
      </Link>
      ...
      <ul class="hidden md:flex space-x-4">
        ...
        <NavLink :href="route('articles')" :active="$page.component === 'Articles'">Articles</NavLink>
        <NavLink :href="route('submit')" :active="$page.component === 'Submit'">Submit</NavLink>
        ...
        <NavLink :href="route('login')" :active="$page.component === 'Login'">Login</NavLink>
        <DesktopNavLinks v-if="$page.props.auth.user"/>
      </ul>

      <button @click="toggle" class="md:hidden focus:outline-none">
        <svg v-if="!isOpen" class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24">
    ...
    <transition name="slide">
      <div v-if="isOpen" class="md:hidden shadow-md p-4">
        <ul class="space-y-3 text-center">
          ...
          <li><Link :href="route('articles')" @click="toggle">Articles</Link></li>
          <li><Link :href="route('submit')" @click="toggle">Submit</Link></li>
